...
class NodeSlb(models.Model):
    ...
    def get_assets(self):
        from .asset import AssetSlb
        assets = AssetSlb.objects.filter(nodes__id=self.id)
        return assets
    ...
    def get_all_assets(self):
        from .asset import AssetSlb
        if self.is_root():
            assets = AssetSlb.objects.all()
        else:
            nodes = self.get_family()
            assets = AssetSlb.objects.filter(nodes__in=nodes).distinct()
        return assets
