...
class NodeRedis(models.Model):
    ...
    def get_assets(self):
        from .asset import AssetRds
        assets = AssetRds.objects.filter(nodes__id=self.id)
        return assets
    ...
    def get_all_assets(self):
        from .asset import AssetRds
        if self.is_root():
            assets = AssetRds.objects.all()
        else:
            nodes = self.get_family()
            assets = AssetRds.objects.filter(nodes__in=nodes).distinct()
        return assets
